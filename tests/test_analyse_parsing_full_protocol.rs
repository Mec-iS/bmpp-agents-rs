        ...
        for protocol in &ast.children {
            if protocol.node_type == AstNodeType::Protocol {
                ...
                if let Some(roles_section) = protocol.get_roles_section() {
                    ...
                    let role_info: Vec<_> = roles
                        .iter()
                        .filter_map(|r| r.get_role_decl_info())
                        .collect();

                    assert!(
                        role_info.contains(&("Producer".to_string(), "Produces data".to_string()))
                    );
                    assert!(
                        role_info.contains(&("Consumer".to_string(), "Consumes data".to_string()))
                    );
                }
                ...
                if let Some(params_section) = protocol.get_parameters_section() {
                    ...
                    assert!(param_info.contains(&(
                        "data".to_string(),
                        "String".to_string(),
                        "The data to be transferred".to_string()
                    )));
                    assert!(param_info.contains(&(
                        "result".to_string(),
                        "Bool".to_string(),
                        "Processing result".to_string()
                    )));
                }
                ...
                if let Some(interactions_section) = protocol.get_interactions_section() {
                    ...
                    assert_eq!(
                        info.parameter_flows[0],
                        ("out".to_string(), "data".to_string())
                    );
                }
            }
        }
...
DisplayTest <Protocol>("Testing Display trait on sections") {
    ...
    fn test_loaded_file_protocol_validation() -> Result<()> {
        ...
        assert!(
            ...
            "Program should contain at least one protocol"
        );

        let protocol_names: Vec<String> = ast
            .children
            .iter()
        ...
        for protocol in &ast.children {
            if protocol.node_type == AstNodeType::Protocol {
                ...
                assert!(
                    protocol.get_roles_section().is_some(),
                    "Protocol {} should have roles section",
                    name
                );
                assert!(
                    protocol.get_parameters_section().is_some(),
                    "Protocol {} should have parameters section",
                    name
                );
                assert!(
                    protocol.get_interactions_section().is_some(),
                    "Protocol {} should have interactions section",
                    name
                );
            }
        }
